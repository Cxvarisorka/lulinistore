...
function Cart() {
    ...
    totalItems.current = cartItems.reduce((total, item) => total + item.quantity, 0);
    ...
    const handlePromoClick = function () {
      ...
      if (codeIndex !== -1 && promoCodes[codeIndex].usage > 0 && totalPrice > 0) { // Check if promo code exists and has usage left
        ...
        updatedPromoCodes[codeIndex].usage = updatedPromoCodes[codeIndex].usage - 1; // Update the usage of the promo code
  ...
  return (
    <div className="cart-page">
      ...
      {cartItems.length === 0 && (
        <div style={{width: '100%', height:'200px', display:'flex', textAlign:'center',justifyContent:'center'}} className="empty-cart-message">
